...
export class ConnectedContextWebProvider {
	...
	public static async render(extensionUri: Uri, loadInstance: loader.Load | undefined, context: ConnectedContext | undefined): Promise<void> {
		// get the loader instance
		let load = loadInstance ? loadInstance : loader.Load.getLoader();
	...
	private async distributeMessages(selectedContext: ConnectedContext): Promise<void> {
		// set the context and retrieve all managedclusters and klusterlets resources
		this.load.setContext(selectedContext);
		let managedClusters = await this.load.getCrs('ManagedCluster');
		let klusterlets = await this.load.getCrs('Klusterlet');
		...
		if (managedClusters.length > 0 ){
			console.debug('tomer debug got here 0');
			// populate the ManagedClusters ui component
			crsDistributions.push(this.distributeCrs('ManagedCluster', managedClusters));
			...
			['ManifestWork', 'Placement', 'PlacementDecision', 'ManagedClusterSet', 'ManagedClusterAddOn', 'ClusterManager', 'SubscriptionReport']
				.forEach(kind => crsDistributions.push(this.distributeCrsForKind(kind)));
		}

		// a cluster with Klusterlet resources, acts as a spoke cluster
		if (klusterlets.length > 0 ){
			// populate the Klusterlets ui component
			crsDistributions.push(this.distributeCrs('Klusterlet', klusterlets));
			// populate common crs used by the spoke cluster
			['AppliedManifestWork', 'SubscriptionStatus'].forEach(kind => crsDistributions.push(this.distributeCrsForKind(kind)));
		}
